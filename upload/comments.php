...
if ( strlen($body) > $site_config['floodsize'] )
    show_error_msg("Error", "Your comment is too big, only $site_config['floodsize'] characters are allowed.", 1);
    $res = SQL_Query_exec("SELECT COUNT(*) FROM `comments` WHERE user = $CURUSER[id] AND added > '" . get_date_time(gmtime() - ($site_config['floodtime'] * 60)) . "'");
    $row = mysql_fetch_row($res);
    
if ($row[0] > $site_config['floodlimit'])
    show_error_msg("Error", "You are only prohibited $site_config[floodlimit] comments in $site_config[floodtime] minutes.", 1);
		//===| End
