...
if (!isset($_POST['template']) && !isset($_POST['change']) && (!isset($_POST['userid']) || !is_valid_id($_POST['userid']))) 
...
//===| Anti-Flood
    if ( strlen($msg) > $site_config['floodsize'] )
        show_error_msg("Error", "Your message is too big, only $site_config['floodsize'] characters are allowed.", 1);
    ...
    if ($row[0] > $site_config['floodlimit'])
        show_error_msg("Error", "You are only prohibited $site_config[floodlimit] messages in $site_config[floodtime] minutes.", 1);
    //===| End
    ...
    $postid = mysql_insert_id() or show_error_msg(T_("FORUM_ERROR"),"Post id n/a");

    //===| Anti-Flood
    $res = SQL_Query_exec("SELECT COUNT(*) FROM `forum_posts` WHERE userid = $CURUSER[id] AND added > '" . get_date_time(gmtime() - ($site_config['floodtime'] * 60)) . "'");
    $row = mysql_fetch_row($res);
    
if ($row[0] > $site_config['floodlimit'])
    show_error_msg("Error", "You are only prohibited $site_config[floodlimit] forumposts in $site_config[floodtime] minutes.", 1);
    //===| End
...
if (is_array($_POST['remove'])) $tmp[] = key($_POST['remove']);
	...
	foreach($_POST['msgs'] as $key => $value) if (is_valid_id($key)) $tmp[] = $key;
	$msgs = implode(', ', $tmp);
...
if ($msgs) {
	...
	$error = "No messages marked as read";
}
...
##########################################################################-Compose START-###########################################################################	
	echo "<center><div id='tablebox'><br /><table class='table_mb' width='100%' border='1' align='center' cellpadding='0' cellspacing='0'></center>";
if (!isset($_GET['templates'])) {
...
#############################################################################################################################
	...
	echo "<table class='table_mb' width='100%' border='1' align='center' cellpadding='4' cellspacing='0'>";
if (!isset($_GET['templates'])) 
	$output .= "<input type=\"submit\" name=\"send\" value=\"".T_("SEND")."\" />&nbsp;<label><sub><input type=\"checkbox\" name=\"save\" checked='checked' /></sub>".T_("SAVE_COPY")."</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"submit\" name=\"draft\" value=\"".T_("SAVE_DRAFT")."\" />&nbsp;";
...
if (isset($_GET['inbox']) && is_valid_id($arr["sender"]))   
	$display .= "<input type=\"submit\" name=\"compose[$arr[id]]\" value=\"".T_("REPLY")."\" />&nbsp;\n";
else
	
if (isset($_GET['draft']) || isset($_GET['templates'])) 
	$display .= "<input type=\"submit\" name=\"compose[$arr[id]]\" value=\"".T_("EDIT_PM")."\" />&nbsp;\n";
if (isset($_GET['inbox']) && $arr['unread'] == 'yes') 
	$display .= "<input type=\"submit\" name=\"mark[$arr[id]]\" value=\"".T_("MARK_AS_READ")."\" />&nbsp;\n";
...
if (isset($_GET['inbox']) && $unread) 
	$buttons .= "&nbsp;<input type=\"button\" value=\"".T_("MARK_SELECTED_AS_READ")."\" onclick=\"this.form.mark.disabled=!this.form.mark.disabled;\" /><input type=\"submit\" name=\"mark\" value=\"".T_("DEL_CONFIRM")."\" disabled=\"disabled\" />";
