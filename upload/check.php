...
if (!is_numeric($site_config["default_theme"]))
	...
	$res = SQL_Query_exec("SELECT uri FROM stylesheets WHERE id=$site_config[default_theme]");
...
if (!is_numeric($site_config["default_language"]))
	...
	$res = SQL_Query_exec("SELECT uri FROM languages WHERE id=$site_config[default_language]");
